class Admin::MeilisearchController < ApplicationController
  ...
  def index
    @current_import = ImportRun.active.order(created_at: :desc).first
    @recent_imports = ImportRun.where.not(status: %w[pending preparing importing swapping]).order(created_at: :desc).limit(5)

    repo = MeilisearchRepository.new(read_timeout: 10)
  ...
  def start_import
    if ImportRun.active.exists?
      redirect_to admin_status_path, alert: "An import is already running."
      return
    end

    run = ImportRun.create!(status: "pending", started_at: Time.current)
    FullImportJob.perform_later(run.id)
    redirect_to admin_status_path, notice: "Import started."
  end

  def cancel_import
    run = ImportRun.active.order(created_at: :desc).first
    if run
      run.cancel!
      redirect_to admin_status_path, notice: "Import cancelled."
    else
      redirect_to admin_status_path, alert: "No active import to cancel."
    end
  end
  ...
  def add_manual_ban
    ...
    redirect_to admin_bans_path, notice: "Banned #{ip}"
  rescue => e
    redirect_to admin_bans_path, alert: "Invalid: #{e.message}"
  end
